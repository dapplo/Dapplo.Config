...
namespace Dapplo.Config.ConfigExtensions
{
    ...
    internal class ShallowCloneableExtension<TProperty> : IShallowCloneable, IConfigExtension<TProperty>
    {
        private readonly ConfigurationBase<TProperty> _parent;

        public ShallowCloneableExtension(ConfigurationBase<TProperty> parent)
        {
            _parent = parent;
        }
        ...
        {
            var typeToClone = _parent.GetType();
            var clonedValue = Activator.CreateInstance(typeToClone) as ConfigurationBase<TProperty>;
            clonedValue?.Initialize(typeToClone);
