...
namespace Dapplo.Ini
{
    ...
    public sealed class IniConfig : IServiceProvider, IDisposable
    {
        ...
        public object GetService(Type configType)
        {
            ...
            // Some information for assisting with tracking issues
            if (Log.IsVerboseEnabled())
            {
                Log.Verbose().WriteLine("{0} is not assignable to {1}", configType.AssemblyQualifiedName, typeof(IIniSection).AssemblyQualifiedName);
            }
            ...
            if (Log.IsVerboseEnabled())
            {
                Log.Verbose().WriteLine("{0} is not assignable to {1}", configType.AssemblyQualifiedName, typeof(IIniSubSection).AssemblyQualifiedName);
            }
            return null;
        }
        ...
        {
            if (typeof(IIniSection).IsAssignableFrom(subsectionType))
            {
